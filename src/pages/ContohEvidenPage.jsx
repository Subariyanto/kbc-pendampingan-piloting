...
function TemplateModal({ item, madrasah, settings, cetak, onClose }) {
 return <Modal open onClose={onClose} title={'Pratinjau G�� ' + item.judul} size="xl" footer={<><button className="btn-ghost" onClick={onClose}>Tutup</button><button className="btn-primary" onClick={() => printPrintArea({ title: item.judul })}>=��� Unduh / Cetak PDF</button></>}><div className="print-area bg-white p-6"><Template item={item} madrasah={madrasah} settings={settings} cetak={cetak}/></div></Modal>
}
...
function FormatUmum({ item }) {
  return <p className="text-center text-slate-500 py-8">Template "{item.judul}" G�� format dapat disesuaikan oleh madrasah.</p>
}
